...
export default function slackin ({
  ...
  if (gcaptcha_sitekey && !gcaptcha_secret) throw new Error('Must provide a `gcaptcha_secret`.')
  if (gcaptcha_secret && !gcaptcha_sitekey) throw new Error('Must provide an `gcaptcha_sitekey`.')
  ...
  app.get('/', (req, res) => {
    ...
    let page = dom('html',
      dom('head',
        ...
        gcaptcha_sitekey && dom("script src=https://www.google.com/recaptcha/api.js"),
  ...
  app.post('/invite', json(), (req, res) => {
    ...
    if(!gcaptcha_sitekey && (captcha_response == undefined || !captcha_response.length)){
      return res
      .status(400)
