...
// realtime updates
var socket = new EventSource(data.path + 'events');
socket.addEventListener('total', function(event) {
  update('total', parseInt(event.data, 10));
});
socket.addEventListener('active', function(event) {
  update('active', parseInt(event.data, 10));
});
